#!/usr/bin/sh

_print_key() {
  cat << 'EOF'
Primary key fingerprint: 1169 DF9F 2273 4F74 3AA5  9232 A9A2 62FF 6760 41BA
DSA key ID 676041BA
pub   1024D/676041BA 2008-06-13
uid                  Cygwin <cygwin@cygwin.com>
sub   1024g/A1DB7B5C 2008-06-13

-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1.4.9 (Cygwin)


=XD1x
-----END PGP PUBLIC KEY BLOCK-----
EOF
}

cp -f cyg-apt /usr/bin/ || exit 1

[ -d /etc/setup/gnupg ] || mkdir /etc/setup/gnupg
_print_key | gpg --import --no-permission-warning --no-secmem-warning --homedir "/etc/setup/gnupg" \
  && chmod +r "/etc/setup/gnupg"/* \
  || exit 1

( HOME=/etc/skel /usr/bin/cyg-apt setup ) || exit 1

cyg-apt setup

